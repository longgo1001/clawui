...
def create_tools():
    return [
        ...
        {"name": "cdp_fill", "description": "Fill a form field by its label, placeholder, or aria-label (no CSS selector needed). Example: cdp_fill('Email', 'user@example.com')", "input_schema": {"type": "object", "properties": {"label": {"type": "string", "description": "Label text, placeholder, or aria-label of the input field"}, "text": {"type": "string", "description": "Text to fill in"}}, "required": ["label", "text"]}},
...
def run_agent(task: str, max_steps: int = 30, model: str = "claude-sonnet-4-20250514",
    ...
    for step in range(max_steps):
        ...
        for block in response["raw_content"]:
            if block.type == "text":
                ...
                logger.info("Tool: %s(%s)", block.name, json.dumps(block.input, ensure_ascii=False)[:100])
    ...
    if stats["tools"]:
        ...
        for tname, tdata in sorted(stats["tools"].items()):
            logger.info("  %s: %din + %dout (%d calls)", tname, tdata["input_tokens"], tdata["output_tokens"], tdata["calls"])
    if stats["phases"]:
