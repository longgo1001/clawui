...
# Add project root to path so we can import clawui.* as top-level packages
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from clawui.agent import create_tools, execute_tool
from clawui.github_integration import create_github_repo
...
class TestGithubIntegration(unittest.TestCase):
    ...
    def test_execute_with_mock_token_success(self):
        ...
        with patch('clawui.github_integration.get_github_token') as mock_token, \
             patch('clawui.github_integration.create_repo_via_api') as mock_api:
    ...
    def test_execute_with_mock_gh_success(self):
        ...
        with patch('clawui.github_integration.get_github_token') as mock_token, \
             patch('clawui.github_integration.is_gh_authenticated') as mock_gh_auth, \
             patch('clawui.github_integration.create_repo_via_gh_cli') as mock_gh:
    ...
    def test_execute_with_mock_cdp_success(self):
        ...
        with patch('clawui.github_integration.get_github_token') as mock_token, \
             patch('clawui.github_integration.is_gh_authenticated') as mock_gh_auth, \
             patch('clawui.github_integration._ensure_cdp_client') as mock_cdp_client, \
             patch('clawui.github_integration.create_repo_via_cdp') as mock_cdp:
            mock_token.return_value = None
